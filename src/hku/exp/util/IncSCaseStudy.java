...
public class IncSCaseStudy {
	...
	public IncSCaseStudy(int graph[][], String nodes[][], TNode root, int core[], String ccsFile){
		this.graph = graph;
		this.nodes = nodes;
		this.root = root;
		this.core = core;
...
//		System.out.println("IncS-queryId:" + queryId);
		...
		this.saver = new CCSSaver(ccsFile + "-variant-" + queryId);
...
//				System.out.println("], coreness:" + entry.getValue());
				...
				if(ccsSet.size() > 1){
					...
					validKwList.add(kws);
					validCoList.add(min);
					System.out.println("queryId:" + queryId + " A community with size = " + ccsSet.size());
					if(ccsSet.size() <= 10)   saver.save(graph, nodes, queryId, ccsSet, kws);
				}else{
...
//		int queryId = 152532;//jim gray
		...
		IncSCaseStudy query = new IncSCaseStudy(graph, nodes, root, core, Config.dblpCCS);
		query.query(queryId);
	}
}