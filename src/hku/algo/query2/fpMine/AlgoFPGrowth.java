 ...
 * FPGrowth is described here: <br/>
 * <br/>
 * 
 * Han, J., Pei, J., & Yin, Y. (2000, May). Mining frequent patterns without
 * candidate generation. In ACM SIGMOD Record (Vol. 29, No. 2, pp. 1-12). ACM <br/>
 * <br/>
 * 
 ...
 * method().
 * 
 * Note: Yixiang downloaded it from http://www.philippe-fournier-viger.com/spmf/index.php?link=download.php
 * 
 * @see FPTree
 * @see Itemset
 * @see Itemsets
...
public class AlgoFPGrowth {
	 ...
	 */
	public Itemsets runAlgorithm(List<List<Integer>> transList, int minsupp)
		...
		// this.minSupportRelative = (int) Math.ceil(minsupp *
		// transactionCount);
		//
		...
		// rewritten by Yixiang Fang, on Oct 11, 2015
		final Map<Integer, Integer> mapSupport = new HashMap<Integer, Integer>();
		for (List<Integer> tran : transList) {
			for (int item : tran) {
		...
		this.minSupportRelative = minsupp;// update it as input
...
//		}
		...
		FPTree tree = new FPTree();
		for (List<Integer> tran : transList) {
			List<Integer> treeTran = new ArrayList<Integer>();
			for (int item : tran) {
				if (mapSupport.get(item) >= minSupportRelative) {
					treeTran.add(item);
				}
			...
			// sort item in the transaction by descending order of support
			Collections.sort(treeTran, new Comparator<Integer>() {
				public int compare(Integer item1, Integer item2) {
					// compare the frequency
					int compare = mapSupport.get(item2) - mapSupport.get(item1);
					// if the same frequency, we check the lexical ordering!
...
//		MemoryLogger.getInstance().checkMemory();
	 ...
	 */
	private void fpgrowth(FPTree tree, int[] prefix, int prefixLength,
		...
		if (singlePath && singlePathSupport >= minSupportRelative) {
			...
			saveAllCombinationsOfPrefixPath(fpNodeTempBuffer, position, prefix,
					prefixLength);
		} else {
	 ...
	 */
	private void saveAllCombinationsOfPrefixPath(FPNode[] fpNodeTempBuffer,
			int position, int[] prefix, int prefixLength) throws IOException {
	 ...
	 * @return a map for storing the support of each item (key: item, value:
	 *         support)
	 */
	private Map<Integer, Integer> scanDatabaseToDetermineFrequencyOfSingleItems(
			String input) throws FileNotFoundException, IOException {
	...
	private void saveItemset(int[] itemset, int itemsetLength, int support)
		...
		if (writer != null) {
			...
			// sort the itemset so that it is sorted according to lexical
			// ordering before we show it to the user
	...
	public void printStats() {
		System.out
				.println("=============  FP-GROWTH 0.96r19 - STATS =============");
