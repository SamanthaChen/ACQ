...
//	private String ccsFile = null;
	...
	public BasicGV3(int graph[][], String nodes[][]){
		this.graph = graph;
		this.nodes = nodes;
	...
	public BasicGV3(String graphFile, String nodeFile, String ccsFile) {
...
//		System.out.println("k-core decomposition finished.");
	...
	public void query(int queryId, String inKws[]) {
...
//			System.out.println("ck-core search has been finished, size=" + cKCoreNode.length);
	...
	private void kwFilter(int cKCoreNode[]) {
		//initialize the candidate list, 2015-11-16
		Set<String> qSet = new HashSet<String>();
		for(int j = 1;j < nodes[queryId].length;j ++)   qSet.add(nodes[queryId][j]);
		List<String[]> candList = new ArrayList<String[]>();
		for(int j = 0;j < inKws.length;j ++){
			if(qSet.contains(inKws[j])){
				String kws[] = {inKws[j]};
				candList.add(kws);
			}
		}
